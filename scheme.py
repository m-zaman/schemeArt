...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(target):
        ...
        # BEGIN Question 5A
        value_lst = expressions.second
        env.define(target, scheme_eval(value_lst.first, env))
        return target
        # END Question 5A
        ...
        func_name, formals, body = target.first, target.second, expressions.second

        env.define(func_name, LambdaProcedure(formals, body, env))
...
def check_formals(formals):
    ...
    formals_iter1 = formals

    while formals_iter1 is not nil:

        if not scheme_symbolp(formals_iter1.first):
            raise SchemeError("formal parameters are not all symbols")


        formals_iter2 = formals_iter1.second

        while formals_iter2 is not nil:

            if formals_iter1.first == formals_iter2.first:
                raise SchemeError("formal parameters are repeated")

            formals_iter2 = formals_iter2.second


        formals_iter1 = formals_iter1.second
